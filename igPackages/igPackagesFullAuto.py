# IG Packages
# For 10 Listicle stratergy (Supporting)
# not countVar version because not in use in this code
# fully automate
# before use set the template because template include $countVar$ which is not in use in thi    script
...
def generate_influencer_html(template_html_file, excel_file, output_folder):
    ...
    random_words = ["substantial following", "engagement rates", "content quality & frequency of posting", "brand partnerships"]
    ...
    output_html_file = os.path.join(output_folder, f'top-{niche_for_file}-instagram-influencers-in-{country_for_file}.html')
...
# Example usage
template_html_file = './igPackages/igPackeges5-template.html'  # Path to your updated HTML template file
