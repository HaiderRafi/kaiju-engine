...
# def generate_influencer_html(template_html_file, excel_file, output_html_file, niche_lower, country_lower):
#     # Load the Excel file
...
#     # Get the Niche and Country from the file name or data
#     html_template = html_template.replace('$NicheVar$', niche_lower.capitalize())
#     html_template = html_template.replace('$CountryVar$', country_lower.capitalize())
#     html_template = html_template.replace('$NicheVarC$', niche_lower.lower())
#     html_template = html_template.replace('$CountryVarC$', country_lower.lower())

#     # List of words to randomize for PRandomVar
#     random_words = ["average reel plays", "follower count", " average likes", "male-female follower ratio"]
...
#             # Extract niche and country from file name
#             parts = file_name.replace('.xlsx', '').split('-')
#             if len(parts) >= 4:  # Ensure the file name has enough parts
#                 niche_lower = parts[3]
#                 country_lower = parts[4]
#             else:
#                 print(f"Skipping file '{file_name}' due to invalid format.")
#                 continue

#             output_html_file = os.path.join(output_folder, f'top-{niche_lower}-instagrammers-in-{country_lower}.html')
#             generate_influencer_html(template_html_file, file_path, output_html_file, niche_lower, country_lower)

# # Example usage
# template_html_file = 'demoAutoExcel.html'  # Path to your updated HTML template file
# folder_path = './demoAutoExcel/'  # Folder containing Excel files
...
#     html_template = html_template.replace('$countVar$', str(rounded_count))
#     html_template = html_template.replace('$NicheVar$', niche.capitalize())
#     html_template = html_template.replace('$CountryVar$', country.capitalize())
...
#             # Extract niche and country for output file name
#             parts = file_name.replace('.xlsx', '').split('-')
#             if len(parts) >= 4:  # Ensure the file name has enough parts
#                 niche_lower = parts[3]
#                 country_lower = parts[4]
...
#             output_html_file = os.path.join(output_folder, f'top-{niche_lower}-instagrammers-in-{country_lower}.html')
#             generate_influencer_html(template_html_file, file_path, output_html_file)
...
# -----------------------------------------------------------
# working fine :)
# 5 template with reel plays
# not random version
# only decending
# FULLY AUTOMATE 
# take niche and country for html file direct form sheet :)
# no tention for 2 word niche and country
